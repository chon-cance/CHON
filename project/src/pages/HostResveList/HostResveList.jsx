  ...
  const fetchReservation = async () => {
    try {
      const acc_response = await fetch(
        `http://port-0-chon-m3qz4omzb344e0d7.sel4.cloudtype.app/accommodations/detail?accommodationId=${id}`
      );
      const acc_data = await acc_response.json();
      setAccommodationName(acc_data.name);

      const response = await fetch(
        `http://port-0-chon-m3qz4omzb344e0d7.sel4.cloudtype.app/accommodations/reservations?accommodationId=${id}`
      );
